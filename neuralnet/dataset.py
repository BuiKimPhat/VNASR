...
class SpecAugment(nn.Module):
    def __init__(self, rate, policy=3, freq_mask=15, time_mask=35):
        ...
        self.rate = rate # rate of data applying policy (if using policy 1 or 2)

        # Single Time and Frequency Masking
        self.specaug = nn.Sequential(
            torchaudio.transforms.FrequencyMasking(freq_mask_param=freq_mask),
            torchaudio.transforms.TimeMasking(time_mask_param=time_mask)
        )

        # Double time and frequency masking
    ...
    def policy3(self, x):
        """ Using half single masking - half double masking """
        probability = torch.rand(1, 1).item()
        if probability > 0.5:
...
class Data(torch.utils.data.Dataset):
    ...
    def __init__(self, json_path, sample_rate, n_feats, specaug_rate, specaug_policy,
        ...
        if valid:
            ...
            # Perform spectrum augmentation again if this is not validation stage
            self.audio_transforms = torch.nn.Sequential(
                LogMelSpec(sample_rate=sample_rate, n_mels=n_feats,  win_length=160, hop_length=80),
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            spectrogram = self.audio_transforms(waveform) # return_shape: (channel, feature, time)
